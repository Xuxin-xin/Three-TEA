...
router.post('/reg', (req, res) => {
    ...
    pool.query('INSERT INTO tea_user SET ?', [obj], (err, result) => {
        if (err) throw err;
        console.log(result);
...
router.post('/login', (req, res) => {
    ...
    //执行SQL语句
    pool.query('SELECT * FROM tea_user WHERE uname=? AND upwd=?', [obj.uname, obj.upwd], (err, result) => {
        if (err) throw err;
        console.log(result);
...
router.get('/detail', (req, res) => {
    ...
    //执行SQL语句
    pool.query('SELECT * FROM tea_user WHERE uid=?', [obj.uid], (err, result) => {
        if (err) throw err;
        console.log(result);
...
router.get('/update', (req, res) => {
    ...
    //执行SQL语句
    pool.query('UPDATE tea_user SET ? WHERE uid=?', [obj, obj.uid], (err, result) => {
        if (err) throw err;
        if (result.affectedRows > 0) {
...
router.get('/list', (req, res) => {
    ...
    //执行查询
    pool.query('SELECT * FROM tea_user limit ?,?', [start, obj.count], (err, result) => {
        if (err) throw err;
        console.log(result);
